...
export default function ProfilePage() {
  ...
  // Helper to generate filter style
  const getFilter = (color: string | null, isEyeOrHair = false) =>
    color ? `drop-shadow(0 0 0 ${color}) brightness(0) saturate(100%) sepia(${isEyeOrHair ? 100 : 0}%)` : "none";
          ...
          <div className="relative flex flex-col items-center gap-4">
            <div className="relative">
              <div className="w-24 h-24 rounded-full overflow-hidden bg-purple-200 relative image-rendering-pixelated">
                ...
                {/* Avatar Layers */}
                <img src="/avatar/skin.png" className="absolute inset-0 w-full h-full" style={{ filter: getFilter(user.skinTone) }} />
                <img src="/avatar/fit.png" className="absolute inset-0 w-full h-full" />
                <img src="/avatar/eyes.png" className="absolute inset-0 w-full h-full" style={{ filter: getFilter(user.eyeColor, true) }} />
                <img src="/avatar/hair.png" className="absolute inset-0 w-full h-full" style={{ filter: getFilter(user.hairColor, true) }} />
              </div>
