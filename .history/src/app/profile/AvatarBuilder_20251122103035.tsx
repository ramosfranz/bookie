...
export default function AvatarBuilder({ onClose, onSave }: AvatarBuilderProps) {
  const [skinTone, setSkinTone] = useState<string | null>(null); // null = original
  const [eyeColor, setEyeColor] = useState<string | null>(null);
  const [hairColor, setHairColor] = useState<string | null>(null);

  const skinOptions = [null, "#ffdbb4", "#f1c27d", "#e0ac69", "#c68642", "#8d5524"];
  const eyeOptions = [null, "#000000", "#6b4f2c", "#4fa0d4", "#7fdf59"];
  const hairOptions = [null, "#ebba5e", "#c13037", "#6b4f2c", "#000000", "#ffffff"];
  const hairStyles = [1, 2, 3, 4, 5]; // visual only

  const getFilter = (color: string | null, isEyeOrHair = false) =>
    color
      ? `drop-shadow(0 0 0 ${color}) brightness(0) saturate(100%) sepia(${isEyeOrHair ? 100 : 0}%)`
      ...
      <div className="bg-white p-6 rounded-xl shadow-xl w-[700px] relative flex gap-6">
        ...
        <div className="flex-1 flex justify-center items-center">
          <div className="relative w-40 h-40 image-rendering-pixelated">
            <img src="/avatar/skin.png" className="absolute inset-0 w-full h-full" style={{ filter: getFilter(skinTone) }} />
            <img src="/avatar/fit.png" className="absolute inset-0 w-full h-full" />
            <img src="/avatar/eyes.png" className="absolute inset-0 w-full h-full" style={{ filter: getFilter(eyeColor, true) }} />
            <img src="/avatar/hair.png" className="absolute inset-0 w-full h-full" style={{ filter: getFilter(hairColor, true) }} />
          </div>
        </div>
            ...
            <div className="flex gap-2">
              {skinOptions.map((color, idx) => (
                <div
                  ...
                  title={color ? undefined : "Original"}
                />
              ))}
            </div>
          </div>
