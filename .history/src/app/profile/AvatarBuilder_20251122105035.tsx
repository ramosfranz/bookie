...
export default function AvatarBuilder({ onClose, onSave }: AvatarBuilderProps) {
  ...
  // Options: first option is null = original PNG
  const skinOptions = [null, "#e6a78a", "#b2806a", "#c68642", "#8d5524"];
        ...
        <div className="flex-1 flex justify-center items-center">
          <div className="relative w-40 h-40 image-rendering-pixelated">
            <img
              ...
              style={skinTone ? { filter: `drop-shadow(0 0 0 ${skinTone}) brightness(0) saturate(100%) sepia(0%)` } : {}}
            />
            <img src="/avatar/fit.png" className="absolute inset-0 w-full h-full" />
            <img
              src="/avatar/eyes.png"
              className="absolute inset-0 w-full h-full"
              style={eyeColor ? { filter: `drop-shadow(0 0 0 ${eyeColor}) brightness(0) saturate(100%) sepia(100%)` } : {}}
            />
            <img
              src="/avatar/hair.png"
              className="absolute inset-0 w-full h-full"
            ...
            <div className="flex gap-2">
              {skinOptions.map((color, idx) => (
                <div
                  ...
                  style={{ backgroundColor: color ?? baseColors.skin }}
            ...
            <div className="flex gap-2">
              {eyeOptions.map((color, idx) => (
                <div
                  ...
                  style={{ backgroundColor: color ?? baseColors.eye }}
