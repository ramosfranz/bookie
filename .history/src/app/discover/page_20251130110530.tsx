...
interface Book {
  ...
  source: "gutendex" | "openlibrary" | "googlebooks";
  isbn?: string;
...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    ...
    if (libraryTab === "leisure") {
      ...
      // 3. Fetch from Google Books - Fiction with preview
      fetchPromises.push(
        fetch(`https://www.googleapis.com/books/v1/volumes?q=subject:fiction&filter=full&maxResults=40&langRestrict=en`)
          .then((res) => res.json())
          .then((data) => {
      ...
      topics.forEach(topic => {
        fetchPromises.push(
          fetch(`https://gutendex.com/books?topic=${topic}&languages=en`)
            .then((res) => res.json())
            .then((data) => {
              return (data.results || []).slice(0, 12).map((book: any) => ({
                id: `gutendex-${book.id}`,
                title: book.title,
                author: book.authors?.[0]?.name,
                cover: book.formats?.["image/jpeg"],
      ...
      fetchPromises.push(
        fetch(`https://openlibrary.org/search.json?q=subject:(computer+science+OR+mathematics+OR+physics+OR+engineering)&has_fulltext=true&limit=25`)
          .then((res) => res.json())
          .then((data) => {
            return (data.docs || []).map((doc: any) => ({
              id: `openlibrary-${doc.key}`,
      ...
      // 3. Fetch from Google Books - Academic with preview
      const academicSubjects = ["computer+science", "mathematics", "physics", "engineering"];
      academicSubjects.forEach(subject => {
        fetchPromises.push(
          fetch(`https://www.googleapis.com/books/v1/volumes?q=subject:${subject}&filter=full&maxResults=10&langRestrict=en`)
            .then((res) => res.json())
            .then((data) => {
              return (data.items || []).map((item: any) => ({
                id: `googlebooks-${item.id}`,
  ...
  const openBookReader = (book: Book) => {
    ...
    if (book.source === "gutendex") {
      ...
      // For Google Books, use embedded viewer
  ...
  // Initialize Google Books viewer when viewerType is google
  useEffect(() => {
    if (viewerType === "google" && viewerUrl) {
      // Wait for Google Books API to load
      ...
      <main className="flex-1 p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              {filteredBooks.length} books from Gutenberg, Open Library & Google Books
            </p>
          </div>
              ...
              {paginatedBooks.map((book) => (
                ...
                >
                  <div className="relative">
                    ...
                    />
                    <span className={`absolute top-1 right-1 text-xs px-2 py-0.5 rounded ${
                      book.source === 'googlebooks' ? 'bg-red-500 text-white' :
                      book.source === 'openlibrary' ? 'bg-blue-500 text-white' :
                      'bg-green-500 text-white'
                    }`}>
                      {book.source === 'googlebooks' ? 'GB' : book.source === 'openlibrary' ? 'OL' : 'PG'}
                    </span>
                  </div>
        ...
        <div className="mt-8 p-4 bg-white rounded-lg shadow text-sm text-gray-600">
          ...
          <ul className="space-y-1 text-xs">
            <li>
              • <strong>100,000+ Books:</strong> Combined from Project Gutenberg (PG), Open Library (OL), and Google Books (GB)
            </li>
            <li>
              • <strong>No Login Required:</strong> All books open directly in your built-in reader
            </li>
            <li>
              • <strong>Multiple Readers:</strong> HTML/EPUB for Gutenberg, Internet Archive for Open Library, Google Books Embedded Viewer
            </li>
            <li>
              • <strong>Free & Legal:</strong> All content is public domain or has preview access
            </li>
            <li>
              • <strong>Source Tags:</strong> PG = Project Gutenberg, OL = Open Library, GB = Google Books
            </li>
          </ul>
        </div>
      </main>
