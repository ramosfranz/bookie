    ...
    if (libraryTab === "leisure") {
      ...
      fetchPromises.push(
        fetch(`https://gutendex.com/books?topic=fiction&languages=en`)
          .then((res) => res.json())
          .then((data: GutendexResponse) => (data.results || []).slice(0, 100).map((book) => ({
            id: `gutendex-${book.id}`,
            title: book.title,
      ...
      fetchPromises.push(
        fetch(`https://www.googleapis.com/books/v1/volumes?q=fiction&maxResults=40&langRestrict=en&printType=books&filter=partial`)
          .then((res) => res.json())
          .then((data: GoogleBooksResponse) => (data.items || []).map((item) => ({
            id: `googlebooks-${item.id}`,
            title: item.volumeInfo.title,
            author: item.volumeInfo.authors?.[0],
            cover: item.volumeInfo.imageLinks?.thumbnail?.replace('http:', 'https:'),
            year: parseInt(item.volumeInfo.publishedDate?.split("-")[0] || "0") || undefined,
            type: "leisure" as const,
      ...
      topics.forEach(topic => {
        fetchPromises.push(
          fetch(`https://gutendex.com/books?topic=${topic}&languages=en`)
            .then((res) => res.json())
            .then((data: GutendexResponse) => (data.results || []).slice(0, 25).map((book) => ({
              id: `gutendex-${book.id}`,
              title: book.title,
              author: book.authors?.[0]?.name,
              cover: book.formats?.["image/jpeg"]?.replace('http:', 'https:'),
    ...
    if (viewerType === "google" && viewerUrl) {
      const initializeGoogleViewer = () => {
        if (window.google?.books) {
          ...
          googleBooks.setOnLoadCallback(loadViewer);
        }
      };
  ...
  <div className="min-h-screen flex bookieBgVar1">
    ...
    <main className="flex-1 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold" style={{ color: "#f67129" }}>Discover</h1>
        </div>
        <div className="flex items-center gap-2">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            />
            <div className="w-12 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:bg-[#f67129] transition-all"></div>
            <div
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-white rounded-lg shadow hover:bg-[#f67129]/10 transition"
        >
          <Filter size={18} /> <span>Filters</span>
          {selectedGenre !== "All" && (
            <span className="bg-[#f67129] text-white text-xs px-2 py-0.5 rounded-full">1</span>
          )}
        </button>
      </div>
      ...
      {loading ? (
              <div className="flex justify-center items-center h-64 bookieBgVar1">
          <div className="flex flex-col items-center">
            <Image
              ...
              className="mb-2"
            />
            <div className="text-[#f67129] text-lg">
                ...
                >
                  <div className="relative">
                    {book.cover ? (
                      ...
                      <div className="w-full h-40 bg-gradient-to-br from-[#f67129]/20 to-indigo-100 rounded mb-2 flex items-center justify-center">
                        {isResearchPaper ? (
                          <FileText size={48} className="text-[#f67129]" />
                        ) : (
                          <BookOpen size={48} className="text-[#f67129]" />
                        )}
                      </div>
                    )}
                    <span
                      className={`absolute top-1 right-1 text-xs px-2 py-0.5 rounded text-white ${badge.color}`}
                    >
                      {badge.label}
                    </span>
                  </div>
                  ...
                  <button
                    ...
                    className="mt-auto px-3 py-1.5 bg-[#f67129] text-white text-xs rounded hover:bg-[#f67129]/80 flex items-center justify-center gap-1 mb-2"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-[#ffa9b8] text-white text-xs rounded hover:bg-[#ffa9b8]/80 flex items-center justify-center gap-1"
                  >
          ...
          {totalPages > 1 && (
            <div className="flex justify-center gap-2 mt-8">
              <button
                ...
                className="px-4 py-2 rounded-lg bg-white shadow hover:bg-[#f67129]/10 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Previous
              </button>
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg ${
                      currentPage === pageNum
                        ? "bg-[#f67129] text-white"
                        : "bg-white shadow hover:bg-[#f67129]/10"
                    }`}
                  >
                    {pageNum}
                  </button>
    ...
    {viewerUrl && (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
        <div className="bg-white w-full h-full max-w-7xl max-h-[95vh] rounded-xl relative shadow-2xl flex flex-col">
          <div className="bg-[#f67129] text-white px-4 py-3 rounded-t-xl flex items-center justify-between">
              <h3 className="font-semibold truncate">{viewerTitle}</h3>

          <button
            className="hover:bg-[#f67129]/80 rounded-full p-1 transition flex-shrink-0"
            onClick={() => {
              setViewerUrl(null);
              setViewerTitle("");
