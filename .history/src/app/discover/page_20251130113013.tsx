...
interface Book {
  ...
  source: "gutendex" | "openlibrary" | "googlebooks" | "arxiv" | "core";
...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    ...
    if (libraryTab === "leisure") {
      ...
      fetchPromises.push(
        fetch(`https://www.googleapis.com/books/v1/volumes?q=subject:fiction&filter=full&maxResults=40&langRestrict=en&orderBy=newest`)
          .then((res) => res.json())
          .then((data) => {
      ...
      const arxivCategories = ["cs", "math", "physics"];
      arxivCategories.forEach(cat => {
        fetchPromises.push(
          fetch(`https://export.arxiv.org/api/query?search_query=cat:${cat}.*&start=0&max_results=15&sortBy=submittedDate&sortOrder=descending`)
            .then((res) => res.text())
            .then((xmlText) => {
      ...
      // 4. Fetch from CORE - Open Access Papers
      fetchPromises.push(
        fetch(`https://api.core.ac.uk/v3/search/works?q=computer%20science&limit=20&api_key=demo`)
          .then((res) => res.json())
          .then((data) => {
      ...
      <main className="flex-1 p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              {filteredBooks.length} items from {libraryTab === "leisure" ? "Gutenberg, Open Library & Google Books" : "Gutenberg, Open Library, arXiv & CORE"}
            </p>
          </div>
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
              {paginatedBooks.map((book) => {
                ...
                const isResearchPaper = book.source === "arxiv" || book.source === "core";
        ...
        <div className="mt-8 p-4 bg-white rounded-lg shadow text-sm text-gray-600">
          ...
          <ul className="space-y-1 text-xs">
            <li>
              • <strong>Fiction:</strong> Classic books from Project Gutenberg, Open Library archives, and modern previews from Google Books
            </li>
            <li>
              • <strong>Academic:</strong> Research papers from arXiv.org (2M+ papers), CORE (200M+ open access papers), plus academic books
            </li>
            <li>
              • <strong>No Login Required:</strong> All content opens directly in your built-in reader with full PDF access
            </li>
            <li>
              • <strong>Sources:</strong> PG = Project Gutenberg | OL = Open Library | GB = Google Books | arXiv = arXiv.org | CORE = CORE.ac.uk
            </li>
            <li>
