    ...
    <div className="min-h-screen flex bg-indigo-50">
      ...
      <main className="flex-1 p-6">
              ...
              {paginatedBooks.map((book) => (
                ...
                >
                  ...
                  {book.readUrl && (
                    <button
                      onClick={() => {
                        // Extract Internet Archive ID from URL
                        const iaId = book.readUrl?.split('/details/')[1];
                        if (iaId) {
        ...
        <div className="mt-8 p-4 bg-white rounded-lg shadow text-sm text-gray-600">
          ...
          <ul className="space-y-1 text-xs">
            <li>• <strong>Read Online:</strong> Books open in a built-in reader within the app</li>
            <li>• <strong>Borrow:</strong> Some books require creating a free Internet Archive account</li>
            <li>• Books are sourced from Open Library & Internet Archive</li>
            <li>• Reader includes page flip, zoom, and full-screen features</li>
          </ul>
        </div>
      </main>
      ...
      {viewerUrl && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full h-full max-w-7xl max-h-[95vh] rounded-xl relative shadow-2xl">
            <div className="absolute top-0 left-0 right-0 bg-purple-700 text-white px-4 py-3 rounded-t-xl flex items-center justify-between z-10">
              <h3 className="font-semibold">Book Reader</h3>
              <button
            ...
            <iframe
              src={viewerUrl}
              className="w-full h-full rounded-xl pt-12"
              title="Book Viewer"
              allowFullScreen
            />
          </div>
