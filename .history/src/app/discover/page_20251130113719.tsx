...
export default function DiscoverPage() {
    ...
    if (libraryTab === "leisure") {
      ...
      fetchPromises.push(
        fetch(`https://gutendex.com/books?topic=fiction&languages=en`)
          .then((res) => res.json())
          .then((data) => {
            return (data.results || []).slice(0, 30).map((book: any) => ({
              id: `gutendex-${book.id}`,
              title: book.title,
      ...
      fetchPromises.push(
        fetch(`https://openlibrary.org/search.json?q=subject:fiction&has_fulltext=true&limit=30`)
          .then((res) => res.json())
          .then((data) => {
      ...
      // 3. Fetch from Google Books - Modern Fiction with preview
      fetchPromises.push(
        fetch(`https://www.googleapis.com/books/v1/volumes?q=fiction&maxResults=40&langRestrict=en&printType=books`)
          .then((res) => res.json())
          .then((data) => {
            console.log("Google Books Response:", data);
            if (!data.items) {
              console.log("No Google Books items found");
              return [];
            }
      ...
      fetchPromises.push(
        fetch(`https://openlibrary.org/search.json?q=subject:(computer+science+OR+mathematics+OR+physics+OR+engineering)&has_fulltext=true&limit=20`)
          .then((res) => res.json())
          .then((data) => {
            return (data.docs || []).map((doc: any) => ({
              id: `openlibrary-${doc.key}`,
      ...
      // 3. Fetch from arXiv - Research Papers (Physics, Math, CS)
      // Note: arXiv fetches from Computer Science category only to reduce load
      fetchPromises.push(
        fetch(`https://export.arxiv.org/api/query?search_query=cat:cs.AI+OR+cat:cs.LG+OR+cat:math.CO&start=0&max_results=30&sortBy=submittedDate&sortOrder=descending`)
          .then((res) => res.text())
          .then((xmlText) => {
            console.log("arXiv Response received");
            ...
            const papers = Array.from(entries).map((entry) => {
              ...
              const arxivId = id.split("/abs/")[1] || id.split("arxiv.org/abs/")[1];
              ...
              return {
                ...
                pdfUrl: pdfLink,
                abstract: summary.substring(0, 150) + "...",
              };
      ...
      // 4. Remove CORE API (requires paid API key)
      // Instead, we'll rely on arXiv, Google Scholar data, and other open sources
    }
  ...
  const openBookReader = (book: Book) => {
    ...
    } else if (book.source === "arxiv" || book.source === "core") {
      ...
      <main className="flex-1 p-6">
        ...
        <div className="mt-8 p-4 bg-white rounded-lg shadow text-sm text-gray-600">
          ...
          <ul className="space-y-1 text-xs">
            ...
            <li>
              • <strong>Academic:</strong> Research papers from arXiv.org (2M+ papers in AI, CS, Math), plus academic books from Gutenberg & Open Library
            </li>
            <li>
              • <strong>No Login Required:</strong> All content opens directly in your built-in reader with full PDF access
            </li>
            <li>
              • <strong>Sources:</strong> PG = Project Gutenberg | OL = Open Library | GB = Google Books | arXiv = arXiv.org
            </li>
            <li>
              • <strong>Recent Research:</strong> arXiv papers sorted by newest submissions, includes latest academic work
            </li>
          </ul>
        </div>
      </main>
