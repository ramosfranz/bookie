...
import { BookOpen, Search, X, Filter, FileText } from "lucide-react";
import Sidebar from "../components/Sidebar";

// --- TYPESCRIPT DECLARATIONS FOR GOOGLE BOOKS API ---
// This is necessary for TypeScript to recognize the global 'google' object
declare global {
  interface Window {
    google?: {
      books: {
...
interface Book {
  ...
  // UPDATED: Added new data sources for full functionality
  source: "gutendex" | "openlibrary" | "googlebooks" | "arxiv";
  googleBooksId?: string; // Required for Google Books viewer
  pdfUrl?: string; // Required for arXiv papers
  abstract?: string; // Useful for arXiv papers
}
...
export default function DiscoverPage() {
  ...
  // NEW STATE: Tracks whether to use iframe or Google's custom viewer
  ...
  useEffect(() => {
    ...
    if (libraryTab === "leisure") {
      // 1. Fetch from Gutendex (Project Gutenberg) - Fiction
      fetchPromises.push(
        fetch(`https://gutendex.com/books?topic=fiction&languages=en`)
          .then((res) => res.json())
          .then((data) => {
            // INCREASED LIMIT: to 100 books for more results
            return (data.results || []).slice(0, 100).map((book: any) => ({
              id: `gutendex-${book.id}`,
              title: book.title,
              author: book.authors?.[0]?.name,
              // FIX: Use optional chaining for cover, replace http with https for modern browsers
      ...
      // 2. Fetch from Open Library - Popular Fiction
      fetchPromises.push(
        // INCREASED LIMIT: to 100 books for more results
      ...
      // 3. NEW SOURCE: Google Books - Modern Fiction with preview
      fetchPromises.push(
        // ADDED FILTER: `filter=partial` ensures we only get books with a preview
        fetch(`https://www.googleapis.com/books/v1/volumes?q=fiction&maxResults=40&langRestrict=en&printType=books&filter=partial`)
          ...
          .then((data) => {
            return (data.items || []).map((item: any) => ({
              ...
              // FIX: Replace http with https for cover images
      ...
      // 1. Fetch from Gutendex - Academic subjects
      ...
      topics.forEach(topic => {
        fetchPromises.push(
          fetch(`https://gutendex.com/books?topic=${topic}&languages=en`)
            .then((res) => res.json())
            .then((data) => {
              // INCREASED LIMIT: to 25 books per topic
              return (data.results || []).slice(0, 25).map((book: any) => ({
                id: `gutendex-${book.id}`,
                title: book.title,
                author: book.authors?.[0]?.name,
                cover: book.formats?.["image/jpeg"]?.replace('http:', 'https:'),
      ...
      // 2. Fetch from Open Library - Academic
      fetchPromises.push(
        // INCREASED LIMIT: to 50 books for more results
      ...
      // 3. NEW SOURCE: arXiv - Research Papers
      fetchPromises.push(
        // WIDER QUERY & INCREASED LIMIT: broader categories and 100 results
          ...
          .then((xmlText) => {
            ...
            return Array.from(entries).map((entry) => {
              ...
              return {
                ...
                cover: undefined, // Research papers typically don't have covers
                year,
                type: "research" as const,
                subjects: categories,
                formats: {},
                ...
                pdfUrl: pdfLink,
                abstract: summary.substring(0, 200) + "...", // Short abstract preview
              };
            }).filter(paper => paper.title && paper.id); // Filter out invalid entries
          })
          .catch((err) => {
    ...
    Promise.all(fetchPromises)
      .then((results) => {
        ...
        // FIX: Remove duplicates based on title for a cleaner list
  ...
  // --- GOOGLE BOOKS VIEWER SCRIPT LOADING ---
  useEffect(() => {
  ...
  // --- GOOGLE BOOKS VIEWER INITIALIZATION (FIXED) ---
  useEffect(() => {
    if (viewerType === "google" && viewerUrl) {
      // Use a timeout to wait for the initial script creation, then check for API readiness
      // This is a cleaner way to handle the script loading race condition.
      const initializeGoogleViewer = () => {
        if (window.google?.books) {
          // 1. Call load() immediately to fetch necessary libraries
          window.google.books.load();
          
          const loadViewer = () => {
            const viewerDiv = document.getElementById('google-viewer-canvas');
            if (viewerDiv) {
              // viewerUrl here is the Google Book ID (e.g., 'ISBN:0738209214')
              const viewer = new window.google.books.DefaultViewer(viewerDiv);
              viewer.load(viewerUrl);
            }
          };
          
          // 2. Use the official callback to ensure DefaultViewer is available
          window.google.books.setOnLoadCallback(loadViewer);
        } else {
          // If the object isn't ready after a short delay, log a warning (optional)
          console.warn("Google Books API object still not available.");
        }
      };

      // Set a short timeout to ensure the script has parsed and the object exists
  ...
  const openBookReader = (book: Book) => {
    ...
    setViewerUrl(null); // Reset URL first
    ...
    if (book.source === "gutendex") {
      ...
      // Use Google Books ID to initialize the custom viewer
    ...
    <div className="min-h-screen flex bg-indigo-50">
      ...
      <main className="flex-1 p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-bold text-purple-700">Discover Books & Papers</h1>
            <p className="text-sm text-gray-600 mt-1">
              {filteredBooks.length} items from {libraryTab === "leisure" ? "Gutenberg, Open Library & Google Books" : "Gutenberg, Open Library & arXiv"}
            </p>
          </div>
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      {/* FIX: Check for cover and show placeholder if missing */}
                      {book.cover ? (
                        <img
                          src={book.cover}
                          alt={book.title}
                      ...
                      <span className={`absolute top-1 right-1 text-xs px-2 py-0.5 rounded text-white ${badge.color}`}>
                        {badge.label}
                      </span>
                    </div>
                    ...
                    {/* NEW: Display abstract for research papers */}
                    {book.abstract && (
                      <p className="text-xs text-gray-500 mb-2 line-clamp-2">{book.abstract}</p>
                    )}
            ...
            {totalPages > 1 && (
              <div className="flex justify-center gap-2 mt-8">
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  let pageNum;
                  // Simplified pagination logic for centralizing current page
                  if (totalPages <= 5) {
                    pageNum = i + 1;
                  ...
                  // Ensure pageNum is never less than 1 (important for small totalPages)
        ...
        <div className="mt-8 p-4 bg-white rounded-lg shadow text-sm text-gray-600">
          ...
          <ul className="space-y-1 text-xs">
            <li>
              • **Fiction:** Classic books from Project Gutenberg, Open Library archives, and modern books from **Google Books**
            </li>
            <li>
              • **Academic:** Research papers from **arXiv.org** (AI, CS, Math) plus academic books from Gutenberg & Open Library
            </li>
            <li>
              • **No Login Required:** All content opens directly in your built-in reader/viewer.
            </li>
            <li>
              • **Sources:** PG = Project Gutenberg | OL = Open Library | GB = Google Books | **arXiv** = arXiv.org
            </li>
          </ul>
        </div>
      </main>

      {/* Embedded Viewer Modal (Now handles iframe and Google Viewer) */}
      {viewerUrl && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full h-full max-w-7xl max-h-[95vh] rounded-xl relative shadow-2xl flex flex-col">
            <div className="bg-purple-700 text-white px-4 py-3 rounded-t-xl flex items-center justify-between">
              ...
              <button
                ...
                onClick={() => {
                  setViewerUrl(null);
                  setViewerTitle("");
                  setViewerType("iframe"); // Reset viewer type on close
                }}
              >
                <X size={24} />
              </button>
            ...
            {viewerType === "iframe" ? (
              <iframe
                // FIX: Ensure iframe is w-full flex-1 for proper layout
                src={viewerUrl}
              ...
              // This div is where the Google Books Viewer will initialize
