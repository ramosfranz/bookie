...
export default function DiscoverPage() {
  ...
  // --- GOOGLE BOOKS VIEWER INITIALIZATION ---
  useEffect(() => {
    if (viewerType === "google" && viewerUrl) {
      // Small interval to wait for the google.books object to load
      const checkGoogleBooks = setInterval(() => {
        if (window.google?.books) {
          clearInterval(checkGoogleBooks);
          window.google.books.load();
          
          const initialize = () => {
            const viewerDiv = document.getElementById('google-viewer-canvas');
            if (viewerDiv) {
              // viewerUrl here is the Google Book ID
              const viewer = new window.google.books.DefaultViewer(viewerDiv);
              viewer.load(viewerUrl);
            }
          };
          
          // Use Google's callback to ensure the library is fully ready
          window.google.books.setOnLoadCallback(initialize);
        }
      }, 100);

      return () => clearInterval(checkGoogleBooks);
    }
      ...
      <main className="flex-1 p-6">
        ...
        <div className="mt-8 p-4 bg-white rounded-lg shadow text-sm text-gray-600">
          ...
          <ul className="space-y-1 text-xs">
            <li>
              • <strong>Fiction:</strong> Classic books from Project Gutenberg, Open Library archives, and modern books from **Google Books**
            </li>
            <li>
              • <strong>Academic:</strong> Research papers from **arXiv.org** (AI, CS, Math) plus academic books from Gutenberg & Open Library
            </li>
            <li>
              • <strong>No Login Required:</strong> All content opens directly in your built-in reader/viewer.
            </li>
            <li>
              • <strong>Sources:</strong> PG = Project Gutenberg | OL = Open Library | GB = Google Books | **arXiv** = arXiv.org
            </li>
          </ul>
        </div>
      </main>
