...
export default function Sidebar({ activeTab }: { activeTab: string }) {
  ...
  // 🔥 Load username + avatar from Supabase profiles table
  useEffect(() => {
    async function loadProfile() {
      ...
      // Convert email → username (before user sets custom username)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isCollapsed && (
          <p className="mt-3 font-semibold text-purple-700">{username}</p>
        )}
      </Link>
