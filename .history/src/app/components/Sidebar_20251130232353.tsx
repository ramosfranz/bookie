...
export default function Sidebar({ activeTab, user }: SidebarProps) {
  ...
  // Load username and avatar from localStorage if available
  useEffect(() => {
        ...
        <div className="w-12 h-12 rounded-full overflow-hidden relative image-rendering-pixelated">
          ...
          >
            <img
              ...
              style={{ filter: avatarData.skinTone || "" }}
            />
            <img src="/avatar/fit.png" className="absolute inset-0 w-full h-full" />
            <img
              src="/avatar/eyes.png"
              className="absolute inset-0 w-full h-full"
              style={{ filter: avatarData.eyeColor || "" }}
            />
            <img
              src="/avatar/hair.png"
              className="absolute inset-0 w-full h-full"
              style={{ filter: avatarData.hairColor || "" }}
            />
          </div>
        </div>

        {!isCollapsed && (
          <p className="mt-3 font-semibold text-purple-700">{username || user?.username}</p>
        )}
      </Link>
