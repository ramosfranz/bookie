...
import { AvatarData } from "../app/AvatarBuilder"; // adjust path if needed

interface SidebarProps {
  activeTab: string;
  user: AvatarData & { username: string };
}
...
export default function Sidebar({ activeTab, user }: SidebarProps) {
  ...
  <img
    src="/avatar/skin.png"
    className="absolute w-full h-full top-[-25%]" // shift 25% up
    style={{ filter: user.skin?.filter || "" }}
  />
  <img
    src="/avatar/fit.png"
  ...
  <img
    src="/avatar/eyes.png"
    className="absolute w-full h-full top-[-25%]"
    style={{ filter: user.eyes?.filter || "" }}
  />
  <img
    src="/avatar/hair.png"
    className="absolute w-full h-full top-[-25%]"
    style={{ filter: user.hair?.filter || "" }}
  />
</div>
      ...
      <nav className="flex flex-col items-center gap-6 mt-4">
        <Link
          ...
          className={`flex flex-col items-center text-xl ${
            activeTab === "dashboard" ? "text-purple-600" : "text-gray-500 hover:text-gray-700"
          }`}
        >
          📚
        ...
        <Link
          ...
          onClick={handleLinkClick}
          className={`flex flex-col items-center text-xl ${
            activeTab === "discover" ? "text-purple-600" : "text-gray-500 hover:text-gray-700"
          }`}
        >
          🔍
        ...
        <Link
          ...
          onClick={handleLinkClick}
          className={`flex flex-col items-center text-xl ${
            activeTab === "chat" ? "text-purple-600" : "text-gray-500 hover:text-gray-700"
          }`}
        >
          💬
